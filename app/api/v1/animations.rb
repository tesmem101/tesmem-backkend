  ...
  class Animations < Grape::API
    ...
    resource :animations do
      ...
      get '/' do
        ...
        searched_animations = Category.where(title: TITLES[:animation]).first.sub_categories.where("lower(title) LIKE ?", "%#{search}%").includes(:stocks).all.map { |sub_c| {id: sub_c.id, name: sub_c.title, data: serialize_collection(sub_c.stocks, serializer: StockSerializer)}}
